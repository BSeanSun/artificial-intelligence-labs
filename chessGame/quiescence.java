...
public class quiescence implements ChessAI{
	int maxdepth;
	...
	public quiescence(int maxdepth){
	...
	int quiesce(int alpha, int beta,Position p, int player) {
        int score = utility(p,player);
        if (score >= beta) 
        	return beta;
        if(alpha<score)
        	alpha = score;
        ...
        for (short m :p.getAllCapturingMoves()) {
            ...
            score = -quiesce(-beta,-alpha,p,player);
            p.undoMove();
	...
	public int max_value(int alpha, int beta, Position p, int depth){
		if(this.cutoffTest(p, depth)==true){
			return quiesce(alpha,beta,p,maxplayer);
	...
	public int min_value(int alpha, int beta, Position p, int depth){
		if(this.cutoffTest(p, depth)==true){
			return quiesce(alpha,beta,p,minplayer);
