...
public class PRMPlanner {
	...
	public PRMPlanner(int sk, int sx, int sy, int sarmlength,int sample) {
		this.k = sk;
		this.x = sx;
		this.y = sy;
	...
	/*
	HashMap<ArmRobot, ArrayList<ArmRobot>> graph = new HashMap<ArmRobot, ArrayList<ArmRobot>>();
	
	public HashMap<ArmRobot, ArrayList<ArmRobot>> CreateGraph(World check){
		
		double[] theta1 = new double[this.sampleNum];
		double[] theta2 = new double[this.sampleNum];
		for(int i = 0; i<30; i++){
		Random random = new Random();
		theta1[i] = (double)random.nextInt(628)%(628+1)/100;
		theta2[i] = (double)random.nextInt(628)%(628+1)/100;
		}
		...
		for(int j = 0; j<this.sampleNum; j++){
			...
			for(int i = 0; i<this.sampleNum;i++){
				if(j!=i){
				double[] new_config = {x,y,armLength, theta1[i], armLength, theta2[i]};
				...
				if(check.armCollision(new_arm)==false &&
					...
					double time = ap.moveInParallel(current_arm.config, new_arm.config);
					new_arm.setTime(time);
					
					arm_list.add(new_arm);
