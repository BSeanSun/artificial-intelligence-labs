...
public class ChessClient extends Application {
	...
	public void start(Stage primaryStage) {
		...
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new transpositiontable(6));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new ROtranspositiontable(15));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new NullMoveAlphaBeta(4));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new quiescence(3));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new MiniMaX(3));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new ABminiMaX(4));
		//moveMaker[Chess.WHITE] = new AIMoveMaker(new EVAminiMaX(2));
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new RandomAI());
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new NullMoveAlphaBeta(4));
	    //moveMaker[Chess.BLACK] = new AIMoveMaker(new MiniMaX(2));
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new EVAminiMaX(4));
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new ROtranspositiontable(3));
  		//moveMaker[Chess.BLACK] = new AIMoveMaker(new ABminiMaX(3));
		//moveMaker[Chess.BLACK] = new AIMoveMaker(new transpositiontable(3));
		moveMaker[Chess.BLACK] = new AIMoveMaker(new quiescence(3));
	...
	private class GameHandler implements EventHandler<ActionEvent> {
		...
		public void handle(ActionEvent e) {
			...
			if (mover.getState() == Worker.State.READY) {
				...
				if(move==-1000){
					//System.out.println("no moves and move = -1000");
					return;
				}
